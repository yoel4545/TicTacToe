#!/usr/bin/env python3
"""
Tic-Tac-Toe (Tres en línea) - juego funcional en consola.
Opciones: jugador vs jugador o jugador vs CPU (IA con Minimax).
Ejecutar: python tic_tac_toe.py
"""
...
def minimax(board, player, maximizing_player, ai_symbol, human_symbol):
    """Minimax que devuelve (mejor_puntaje, mejor_movimiento).
    Puntajes: +1 victoria AI, -1 victoria humano, 0 empate.
    """
    win = winner(board)
    if win == ai_symbol:
        return 1, None
...
def cpu_move(board, ai_symbol, human_symbol):
    # Si el tablero está vacío, intenta jugar en una esquina o centro
...
def player_move(board, symbol):
    while True:
        try:
            choice = input(f"Turno de {symbol}. Elige casilla (1-9): ")
            idx = int(choice) - 1
...
def play():
    ...
    if mode == 1:
        ...
        current = 'X'  # X siempre empieza
        display_board(board)
        while True:
            if current == human_symbol:
                mv = player_move(board, human_symbol)
                board[mv] = human_symbol
            else:
